...
// event requires it)
type SsgcEventInit<T> = T extends keyof GlobalEventHandlersEventMap
  ? GlobalEventHandlersEventMap[T] extends CustomEvent<
      Record<PropertyKey, unknown>
    >
    ? GlobalEventHandlersEventMap[T] extends CustomEvent<
...
export default class SsgcElement extends LitElement {
  ...
  static version = __SSGC_VERSION__;
  /* eslint-enable */
  ...
  static dependencies: Record<string, typeof SsgcElement> = {};

  constructor() {
    super();
    Object.entries(
      (this.constructor as typeof SsgcElement).dependencies
    ).forEach(([name, component]) => {
      (this.constructor as typeof SsgcElement).define(name, component);
    });
  }
}

export interface ShoelaceFormControl extends SsgcElement {
  // Form attributes
  name: string;
  value: unknown;
  disabled?: boolean;
