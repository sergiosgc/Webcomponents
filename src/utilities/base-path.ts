 ...
 * By default, this script will look for a script ending in webcomponents.js or ssgc-autoloader.js and set the base path
 * to the directory that contains that file. To override this behavior, you can add the data-ssgc attribute to any
 ...
 *
 *   <script src="bundle.js" data-ssgc="/custom/base/path"></script>
 *
...
export function getBasePath(subpath = "") {
  if (!basePath) {
    ...
    const configScript = scripts.find((script) =>
      script.hasAttribute("data-ssgc")
    );

    if (configScript) {
      // Use the data-ssgc attribute
      setBasePath(configScript.getAttribute("data-ssgc")!);
    } else {
      const fallbackScript = scripts.find((s) => {
        return (
          /webcomponents(\.min)?\.js($|\?)/.test(s.src) ||
          /ssgc-autoloader(\.min)?\.js($|\?)/.test(s.src)
        );
      });
      let path = "";
