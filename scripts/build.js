...
const ssgcVersion = JSON.stringify(packageData.version.toString());
...
await nextTask("Generating themes", () => {
  return execPromise(`node scripts/make-themes.js --outdir "${outdir}"`, {
    stdio: "inherit",
  });
});
...
await nextTask(
  `Copying Web Types, Themes, Icons, and TS Types to "${cdndir}"`,
  async () => {
